...
for entry, count in counter.items():
    ...
    data_to_sort.append([entry, count, additional_info, description]) 
#To note from the above code additional_info should correspond to the description of the gene while description should give the Gene name. I'm unsure why
# but if I swap them in the above line their positions in the final display is incorrect. I'm unsure where the mixup/mislabelling is occuring but intend to fix it if time allows.
...
while repeat_process:
    ...
    for entry, count in counter.items():
        description, additional_info = description_data.get(entry, ('N/A', 'N/A'))  #N/A designates where no information was present for the specific ID.
        data_to_sort.append([entry, count, additional_info, description])
    #Again the same issue as in the code above is repeating itself.
    ...
    def main():
        ...
        if response == 'yes':
            ...
            sys.exit() #Exiting the script with a message to let the user know
